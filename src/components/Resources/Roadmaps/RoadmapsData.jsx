const RoadmapsData = [
    {
        id: 1,
        title: "Cyber Security Beginner",
        level: "Beginner",
        desc: "Step by step guide to learn Cyber Security from scratch",
        details: [
            {
                section: "Join the Community",
                resources: [
                    {
                        title: "Discord - Join the Community & Learn with others",
                        url: "https://discord.gg/QHBPq6xP5p",
                    },
                ],
            },
            {
                section: "Essential SkillLevel",
                resources: [
                    {
                        title: "Introduction to Linux",
                        url: "https://youtu.be/iwolPf6kN-k",
                    },
                    {
                        title: "Linux Essentials for Ethical Hackers",
                        url: "https://www.youtube.com/watch?v=1hvVcEhcbLM&t=12602s&pp=ygUSbGludXggIGZvciBoY2FrcmVz",
                    },
                ],
            },
            {
                section: "Computer Networking",
                resources: [
                    {
                        title: "Computer Networking",
                        url: "https://youtu.be/IPvYjXCsTg8?si=-ngLluloP-bcKO-m",
                    },
                ],
            },
            {
                section: "Programming",
                resources: [
                    {
                        title: "HTML",
                        url: "https://www.youtube.com/watch?v=qz0aGYrrlhU",
                    },
                    {
                        title: "JavaScript",
                        url: "https://www.youtube.com/watch?v=W6NZfCO5SIk",
                    },
                    {
                        title: "SQL",
                        url: "https://www.youtube.com/watch?v=p3qvj9hO_Bo",
                    },
                    {
                        title: "Bash Shell Scripting",
                        url: "https://www.youtube.com/watch?v=Zl7npywCB84",
                    },
                    {
                        title: "Python",
                        url: "https://www.youtube.com/watch?v=7utwZYKweho&t=8861s",
                    },
                    {
                        title: "Applied Python",
                        url: "https://youtube.com/playlist?list=PLB7R26sRn2aLhKbDDRtd7wluaX91pqAQD&si=2zKV2iLoaFrTqxFh",
                        isOptional: true,
                    },
                ],
            },
            {
                section: "Cyber Security",
                resources: [
                    {
                        title: "Ethical Hacking Course",
                        url: "https://youtu.be/fNzpcB7ODxQ",
                    },
                    {
                        title: "Open-Source Intelligence",
                        url: "https://youtu.be/qwA6MmbeGNo",
                    },
                ],
            },
            {
                section: "WebApp Pen-testing",
                resources: [
                    {
                        title: "Web App Pentesting",
                        url: "https://youtu.be/X4eRbHgRawI",
                    },
                    {
                        title: "Web App Penetration Testing Tutorials",
                        url: "https://www.youtube.com/playlist?list=PLBf0hzazHTGO3EpGAs718LvLsiMIv9dSC",
                    },
                    {
                        title: "API Pentesting",
                        url: "https://university.apisec.ai/apisec-certified-expert",
                    },
                ],
            },
            {
                section: "Important for Improving skills",
                resources: [
                    {
                        title: "Web Security",
                        url: "https://thecyberhub.org/websecurity",
                    },
                    {
                        title: "TheCyberCTF",
                        url: "https://thecyberhub.org/ctf",
                    },
                    {
                        title: "PortSwigger Academy",
                        url: "https://portswigger.net/web-security/learning-path",
                    },
                    {
                        title: "TryHackMe",
                        url: "https://tryhackme.com/",
                    },
                    {
                        title: "VulnHub",
                        url: "https://www.vulnhub.com/",
                    },
                ],
            },
            {
                section: "Internship for Experience",
                resources: [
                    {
                        title: "Apply for Internship at TheCyberHub",
                        url: "https://www.thecyberhub.org/internship",
                    },
                ],
            },
        ],
    },
    {
        id: 2,
        title: "Bug Hunting",
        level: "Beginner",
        desc: "Step by step guide to learn Bug Hunting from scratch",
        details: [
            {
                section: "Join the Community",
                resources: [
                    {
                        title: "Discord - Join the Community & Learn with others",
                        url: "https://discord.gg/QHBPq6xP5p",
                    },
                ],
            },
            {
                section: "First Complete the Cyber Security Roadmap",
                resources: [
                    {
                        title: "Cyber Security Beginner Roadmap",
                        url: "https://thecyberhub.org/roadmaps/cyber-security-beginner",
                    },
                ],
            },
            {
                section: "Bug Hunting from Scratch - Level 1",
                resources: [
                    {
                        title: "New to bug hunting",
                        url: "https://www.youtube.com/watch?v=hDYqWZ11njU&list=PLbyncTkpno5FAC0DJYuJrEqHSMdudEffw",
                    },
                    {
                        title: "Everything API Hacking",
                        url: "https://www.youtube.com/watch?v=yCUQBc2rY9Y&list=PLbyncTkpno5HqX1h2MnV6Qt4wvTb8Mpol",
                    },
                    {
                        title: "Bug bounty / webapp pentesting tutorials",
                        url: "https://www.youtube.com/playlist?list=PLF7JR1a3dLONdkRYU_8-5OcgOzrWe2549",
                    },
                    {
                        title: "Zero to Bug Bounty Hero!",
                        url: "https://www.youtube.com/watch?v=nXvP8j3QtHI&list=PLbyncTkpno5FaAbJJ7YHvMY9wtJ9fyqCH",
                    },
                    {
                        title: "Reported to Resolved",
                        url: "https://www.youtube.com/watch?v=fywFveRBOE4&list=PLbyncTkpno5GHVJSca8K4Mqfpv52jEWxk",
                    },
                ],
            },
            {
                section: "Bug Hunting - Level 2",
                resources: [
                    {
                        title: "Guide to Failing at Bug Bounties",
                        url: "https://www.youtube.com/playlist?list=PLhfP6zOcRP1f_FyWc_gk1fRz1mxst0QE5",
                    },
                    {
                        title: "What after Recon?",
                        url: "https://www.youtube.com/playlist?list=PLhfP6zOcRP1fXtBtxhTlu9KR-PKDA6biX",
                    },
                    {
                        title: "No BS Guides",
                        url: "https://www.youtube.com/playlist?list=PLhfP6zOcRP1dCTjbENt4ilFKFwFpspOoS",
                    },
                ],
            },
            {
                section: "Bug Hunting - Level 3",
                resources: [
                    {
                        title: "OrwaGodFather Methodology",
                        url: "https://www.youtube.com/playlist?list=PLiLvsecrejRhQ7lOGgZSga47Jwhf5YXwD",
                    },
                    {
                        title: "Live Bug Hunting",
                        url: "https://youtube.com/playlist?list=PLwfoGUoc87CvvBETVO64M9jgFpzvq68GM&si=vjA2GkJYippqQQ_l",
                    },
                ],
            },
            {
                section: "Internship for Experience",
                resources: [
                    {
                        title: "Apply for Internship at TheCyberHub",
                        url: "https://www.thecyberhub.org/internship",
                    },
                ],
            },
        ],
    },
    {
        id: 1,
        title: "Penetration and Vulnerability Tester",
        level: "Beginner",
        desc: "Step by step guide to learn Cyber Security from scratch",
        details: [
            {
                section: "Join the Community",
                resources: [
                    {
                        title: "Discord - Join the Community & Learn with others",
                        url: "https://discord.gg/QHBPq6xP5p",
                    },
                ],
            },
            {
                section: "First Complete the Cyber Security Roadmap",
                resources: [
                    {
                        title: "Cyber Security Beginner Roadmap",
                        url: "https://thecyberhub.org/roadmaps/cyber-security-beginner",
                    },
                ],
            },
            {
                section: "Vulnerability Scanning",
                resources: [
                    {
                        title: "Vulnerability Scanning",
                        url: "https://www.youtube.com/playlist?list=PLBf0hzazHTGNxNNLU6eQ60JvTmlOD-QY9",
                    },
                ],
            },
            {
                section: "Linux Privilege Escalation and Exploitation",
                resources: [
                    {
                        title: "Linux Privilege Escalation",
                        url: "https://www.youtube.com/watch?v=af0PGYaqIWA&list=PLBf0hzazHTGPlod44Eeca2nTnYoRB6h0M",
                    },
                    {
                        title: "Linux Exploitation",
                        url: "https://www.youtube.com/watch?v=i-dQwejj518&list=PLBf0hzazHTGO6Alxf5oduMaUG4W7hbI9a",
                    },
                    {
                        title: "Linux Post Exploitation",
                        url: "https://www.youtube.com/watch?v=ChgqGBwl8NQ&list=PLBf0hzazHTGNc34A3cMOPJGqDTp4d56ZS",
                    },
                ],
            },
            {
                section: "Windows Privilege Escalation and Exploitation",
                resources: [
                    {
                        title: "Windows Privilege Escalation",
                        url: "https://www.youtube.com/watch?v=dSa_mdg3gCg&list=PLBf0hzazHTGMBgq3RXdGDNJjw9iEz-lKz",
                    },
                    {
                        title: "Windows Exploitation",
                        url: "https://www.youtube.com/watch?v=32W6Y8fVFzg&list=PLBf0hzazHTGMT1CxfH1mdaB3kv9EqiVFq",
                    },
                    {
                        title: "Windows Post Exploitation",
                        url: "https://www.youtube.com/watch?v=96k1eqc55jc&list=PLBf0hzazHTGMoQ4B8dXy4kpPoo7ibemEm",
                    },
                ],
            },
            {
                section: "Cloud Penetration Testing",
                resources: [
                    {
                        title: "Cloud Penetration Testing",
                        url: "https://www.youtube.com/watch?v=ITSZ8743MUk&list=PLBf0hzazHTGPsUAdkN3DOyyExRjk8SshU",
                    },
                ],
            },
            {
                section: "Active Directory",
                resources: [
                    {
                        title: "Active Directory Penetration Testing",
                        url: "https://www.youtube.com/watch?v=n3Ow_LKanMo&list=PLBf0hzazHTGPTPemna-KfS9zEx5TL6MGg",
                    },
                ],
            },
            {
                section: "Python 3",
                resources: [
                    {
                        title: "Python 3 for Penetration Testing",
                        url: "https://www.youtube.com/watch?v=UadqiHXfvsg&list=PLBf0hzazHTGM_dncTqO9l-0zUQYP0nNPU",
                    },
                ],
            },
            {
                section: "Internship for Experience",
                resources: [
                    {
                        title: "Apply for Internship at TheCyberHub",
                        url: "https://www.thecyberhub.org/internship",
                    },
                ],
            },
        ],
    },
    {
        id: 3,
        title: "MERN Stack Roadmap",
        level: "beginner",
        desc: "Step by step guide to becoming a Mern Stack Developer",
        details: [
            {
                section: "Join the Community",
                resources: [
                    {
                        title: "Discord - Join the Community & Learn with others",
                        url: "https://discord.gg/QHBPq6xP5p",
                    },
                ],
            },
            {
                section: "Essential Skills",
                resources: [
                    {
                        title: "Git and GitHub for Beginners - Crash Course",
                        url: "https://www.youtube.com/watch?v=RGOj5yH7evk&t=1490s&pp=ygUOZ2l0IGFuZCBnaXRodWI%3D",
                    },
                    {
                        title: "Linux Operating System - Crash Course for Beginners",
                        url: "https://www.youtube.com/watch?v=ROjZy1WbCIA&pp=ygUUbGludXggZnJlZSBjb2RlIGNhbXA%3D",
                    },
                ],
            },
            {
                section: "HTML CSS and JavaScript",
                resources: [
                    {
                        title: "Build a Simple Website with HTML, CSS, JavaScript – Course for Beginners",
                        url: "https://youtu.be/krfUjg0S2uI",
                    },
                    {
                        title: "Project: Build the Google Homepage [Do it yourself]",
                        url: "https://google.com",
                    },
                    {
                        title: "Portfolio Website Tutorial – Frontend Development with HTML, CSS, JavaScript",
                        url: "https://www.youtube.com/watch?v=xV7S8BhIeBo&t=135s&pp=ygUXSFRNTCBDU1MgYW5kIEphdmFTY3JpcHQ%3D",
                    },
                ],
            },
            {
                section: "MERN Stack",
                resources: [
                    {
                        title: "React JS Full Course",
                        url: "https://www.youtube.com/watch?v=b9eMGE7QtTk&t=3132s&pp=ygUScmVhY3QgY3Jhc2ggY291cnNl",
                    },
                    {
                        title: "Learn MERN Stack",
                        url: "https://www.youtube.com/watch?v=-0exw-9YJBo&list=PLillGF-RfqbbQeVSccR9PGKHzPJSWqcsm",
                    },
                ],
            },
            {
                section: "Projects [Do it yourself]",
                resources: [
                    {
                        title: "Build any simple website clone using React JS [eg: Google, Facebook, Twitter, etc]",
                        url: "",
                    },
                    {
                        title: "Project: Add features to the existing MERN Stack Project (Where user can post blog and comments)",
                        url: "",
                    },
                ],
            },
            {
                section: "Open Source Contributions",
                resources: [
                    {
                        title: "Do Open Source Contributions [fix bugs, add features to existing projects]",
                        url: "https://github.com/thecyberworld/TheCyberHUB",
                    },
                ],
            },
            {
                section: "Internship for Experience",
                resources: [
                    {
                        title: "Apply for Internship at TheCyberHub",
                        url: "https://www.thecyberhub.org/internship",
                    },
                ],
            },
        ],
    },
    {
        id: 4,
        title: "Next JS Roadmap",
        level: "beginner",
        desc: "Step by step guide to becoming a Next JS Developer",
        details: [
            {
                section: "Join the Community",
                resources: [
                    {
                        title: "Discord - Join the Community & Learn with others",
                        url: "https://discord.gg/QHBPq6xP5p",
                    },
                ],
            },
            {
                section: "First Complete the MERN Stack Roadmap",
                resources: [
                    {
                        title: "MERN Stack Roadmap",
                        url: "https://thecyberhub.org/roadmaps/mern-stack-roadmap",
                    },
                ],
            },
            {
                section: "Next JS",
                resources: [
                    {
                        title: "Next.js 13 Crash Course | App Directory, React Server Components & More",
                        url: "https://youtu.be/krfUjg0S2u",
                    },
                    {
                        title: "Next.js React Framework Course – Build and Deploy a Full Stack App From scratch",
                        url: "https://youtu.be/KjY94sAKLlw",
                    },
                ],
            },
            {
                section: "Projects [Do it yourself]",
                resources: [
                    {
                        title: "Build any simple website clone using Next JS [eg: Google, Facebook, Twitter, etc]",
                        url: "",
                    },
                    {
                        title: "Convert the existing MERN Stack Project to Next JS Project",
                        url: "",
                    },
                ],
            },
            {
                section: "Open Source Contributions",
                resources: [
                    {
                        title: "Do Open Source Contributions [fix bugs, add features to existing projects]",
                        url: "https://github.com/thecyberworld",
                    },
                ],
            },
            {
                section: "Internship for Experience",
                resources: [
                    {
                        title: "Apply for Internship at TheCyberHub",
                        url: "https://www.thecyberhub.org/internship",
                    },
                ],
            },
        ],
    },
    {
        id: 5,
        title: "React Native Roadmap",
        level: "beginner",
        desc: "Step by step guide to becoming a React Native Developer",
        details: [
            {
                section: "Join the Community",
                resources: [
                    {
                        title: "Discord - Join the Community & Learn with others",
                        url: "https://discord.gg/QHBPq6xP5p",
                    },
                ],
            },
            {
                section: "First Complete the MERN Stack Roadmap",
                resources: [
                    {
                        title: "MERN Stack Roadmap",
                        url: "https://thecyberhub.org/roadmaps/mern-stack-roadmap",
                    },
                ],
            },
            {
                section: "React Native Crash Courses",
                resources: [
                    {
                        title: "React Native Crash Course 2023",
                        url: "https://www.youtube.com/watch?v=mJ3bGvy0WAY&t=3691s&pp=ygUMcmVhY3QgbmF0aXZl",
                    },
                    {
                        title: "React Native App | NFT Marketplace Course",
                        url: "https://www.youtube.com/watch?v=_ivIUCSOZ78",
                    },
                ],
            },
            {
                section: "Projects [Do it yourself]",
                resources: [
                    {
                        title: "Build any simple mobile app clone using React Native [eg: Google, Facebook, Twitter, etc]",
                        url: "",
                    },
                    {
                        title: "Convert the existing MERN Stack Project to React Native Project",
                    },
                ],
            },
            {
                section: "Open Source Contributions",
                resources: [
                    {
                        title: "Do Open Source Contributions [fix bugs, add features to existing projects]",
                        url: "https://github.com/thecyberworld/thecyberhub-app",
                    },
                ],
            },
            {
                section: "Internship for Experience",
                resources: [
                    {
                        title: "Apply for Internship at TheCyberHub",
                        url: "https://www.thecyberhub.org/internship",
                    },
                ],
            },
        ],
    },
];

export default RoadmapsData;
