...
For the purpose of this document we will call that process: _transaction authorization_.
...
Such transaction authorization components should be built using the _What You See Is What You Sign_ principle. When a user authorizes a transaction they need to know what they are authorizing. Based on this principle, an authorization method must permit a user to identify and acknowledge the data that is significant to a given transaction. For example, in the case of a wire transfer: the target account and amount.
