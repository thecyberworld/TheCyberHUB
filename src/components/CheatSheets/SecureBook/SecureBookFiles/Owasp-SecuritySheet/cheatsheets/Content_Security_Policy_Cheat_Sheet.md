...
Using hashes can be a risk approach approach. If you change _anything_ inside the script tag (even whitespace) by, e.g., formatting your code, the hash will be different, and the script won't render.
...
```html
<button id="button1" onclick="doSomething()"></button>
```
